...
function SendDataToDatabase() {
  ...
  try {
    ...
    if (!data || data.length < 2) {
      Logger.log('No data found in Cameras sheet or insufficient rows.');
      return;
    }
    ...
    for (let row = 1; row < data.length; row++) { // Start at 1 to skip header
      try {
        ...
        Logger.log(`Error processing row ${row + 1}: ${error.message}`);
        continue;
      }
    }
    ...
    // Limit processing to prevent timeouts (process max 50 rows per execution)
    const maxRowsToProcess = 50;
    if (unprocessedRows.length > maxRowsToProcess) {
      Logger.log(`Limiting processing to ${maxRowsToProcess} rows to prevent timeout. ${unprocessedRows.length - maxRowsToProcess} rows will be processed in next execution.`);
      unprocessedRows = unprocessedRows.slice(0, maxRowsToProcess);
    }
    ...
    for (const rowObj of unprocessedRows) {
      try {
        ...
        // Fetch location from column G (index 6) of Cameras sheet with bounds checking
        let location = '';
        try {
          if (data[rowObj.rowNum - 1] && data[rowObj.rowNum - 1][6] !== undefined) {
            location = data[rowObj.rowNum - 1][6];
          }
        } catch (error) {
          Logger.log(`Error fetching location for row ${rowObj.rowNum}: ${error.message}`);
          location = '';
        }
        Logger.log(`Fetched location for barcode ${rowObj.barcode}: ${location}`);

        // Helper to update status and location in external sheet with retry logic
        function updateStatusAndLocationInExternalSheet(sheetId, sheetName, barcode, status, location) {
          ...
          while (retryCount < maxRetries) {
            try {
              Logger.log(`Attempting to update status and location in external sheet: ${sheetName} (${sheetId}) for barcode: ${barcode} (attempt ${retryCount + 1})`);
              ...
              retryCount++;
              Logger.log(`Error updating external sheet (attempt ${retryCount}): ${error.message}`);
              if (retryCount >= maxRetries) {
                Logger.log(`Failed to update external sheet after ${maxRetries} attempts: ${error.message}`);
                return;
              }
              // Wait before retry
        ...
        Logger.log(`Error processing row ${rowObj.rowNum}: ${error.message}`);
        // Continue processing other rows even if one fails
        continue;
      }
    }
    
    Logger.log(`SendDataToDatabase completed successfully. Processed ${unprocessedRows.length} rows.`);
    
  } catch (error) {
    Logger.log(`SendDataToDatabase failed with error: ${error.message}`);
